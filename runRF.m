function [errtr,errts,prox,trees,predictts,varimp,scale]=...
    runRF(trainData,testData,classwt,cat0,msm,runParam,impOpt,proCom,missingVal,saveForest,runForest,outParam,seed)
if(trainData==testData)
    data=textread(trainData);
    [m,n]=size(data);
    randomNum=randperm(m);%returns a row vector containing a random permutation of the integers from 1 to n inclusive.
    randomData=data(randomNum,:);
    ntest=ceil(m/3);
    ntrain=m-ntest;
    x=(randomData(1:ntrain,1:n-1));%data
    x=x';
    cl=(randomData(1:ntrain,n));%labels
    xts=(data(ntrain+1:m,1:n-1));%test data
    xts=xts';
    clts=(data(ntrain+1:m,n));%test data labels
else
...
uniquecl=unique(cl);%labelsÖÖÀàÊı
...
[errtr,errts,prox,trees,predictts,varimp,scale]=...
    RF(nclass,x,cl,xts,clts,classwt,cat0,msm,runParam,proCom,missingVal,saveForest,runForest,outParam,seed,dataName);
errtr=errtr';
errts=errts';
prox=prox';
predictts=predictts';
